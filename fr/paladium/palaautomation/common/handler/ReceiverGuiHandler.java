...
import fr.paladium.palaautomation.client.ui.UIReceiver;
...
public class ReceiverGuiHandler extends GHandler {
  @SideOnly(Side.SERVER)
  public static void open(EntityPlayerMP player, int x, int y, int z) {
    player.openGui(PalaAutomationMod.getInstance(), CommonProxy.getInstance().getReceiverHandlerId(), player.field_70170_p, x, y, z);
  }
  ...
  public Container getServerGuiElement(EntityPlayer entityPlayer, World world, int x, int y, int z) {
    ...
    if (!(tile instanceof TileEntityReceiver))
      return null; 
    return (Container)new ContainerReceiver((TileEntityReceiver)tile, entityPlayer);
  }
  
  @SideOnly(Side.CLIENT)
  public GuiScreen getClientGuiElement(EntityPlayer entityPlayer, World world, int x, int y, int z) {
    return (GuiScreen)ZUI.open((UI)new UIReceiver((TileEntityReceiver)world.func_147438_o(x, y, z)));
  }
}


/* Location:              E:\Paladium\!\fr\paladium\palaautomation\common\handler\ReceiverGuiHandler.class
