...
public class Handler implements IMessageHandler<CSPacketFishingInteraction, IMessage> {
  ...
  public IMessage onMessage(CSPacketFishingInteraction message, MessageContext ctx) {
    ...
    if (category == null)
      return null; 
    if (CSPacketFishingInteraction.access$000(message) < 0) {
      FishingManager.close((EntityPlayer)player);
      return null;
    } 
    if (CSPacketFishingInteraction.access$000(message) >= category.getSections().size()) {
      FishingManager.close((EntityPlayer)player);
      return null;
    } 
    FishingSection section = category.getSections().get(CSPacketFishingInteraction.access$000(message));
...
/* Location:              E:\Paladium\!\fr\paladium\palajobs\core\packets\client\CSPacketFishingInteraction$Handler.class
