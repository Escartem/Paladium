package fr.paladium.palajobs.core.pojo.boss;

import net.minecraft.nbt.NBTTagCompound;

public class BossPlayerStat {
  private String name;
  
  private int kills;
  
  private double damage;
  
  private long deposit;
  
  public BossPlayerStat(String name, int kills, double damage, long deposit) {
    this.name = name;
    this.kills = kills;
    this.damage = damage;
    this.deposit = deposit;
  }
  
  public String getName() {
    return this.name;
  }
  
  public int getKills() {
    return this.kills;
  }
  
  public double getDamage() {
    return this.damage;
  }
  
  public long getDeposit() {
    return this.deposit;
  }
  
  public BossPlayerStat(String name) {
    this.name = name;
    this.kills = 0;
    this.damage = 0.0D;
    this.deposit = 0L;
  }
  
  public BossPlayerStat(NBTTagCompound compound) {
    this.name = compound.func_74779_i("name");
    this.kills = compound.func_74762_e("kills");
    this.damage = compound.func_74769_h("damage");
    this.deposit = compound.func_74763_f("deposit");
  }
  
  public NBTTagCompound toNBT() {
    NBTTagCompound compound = new NBTTagCompound();
    compound.func_74778_a("name", this.name);
    compound.func_74768_a("kills", this.kills);
    compound.func_74780_a("damage", this.damage);
    compound.func_74772_a("deposit", this.deposit);
    return compound;
  }
  
  public void increaseDeposit(long deposit) {
    this.deposit += deposit;
  }
  
  public void addDamage(double damage) {
    this.damage += damage;
  }
  
  public void addKill() {
    this.kills++;
  }
}


/* Location:              E:\Paladium\!\fr\paladium\palajobs\core\pojo\boss\BossPlayerStat.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */