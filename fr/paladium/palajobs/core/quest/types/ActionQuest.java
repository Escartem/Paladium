...
public class ActionQuest extends AbstractQuest {
  ...
  public static ActionQuestBuilder builder() {
    return new ActionQuestBuilder();
  }
  
  public static class ActionQuestBuilder {
    private String id;
  ...
  public ActionQuest(String id, JobType job, String name, String description, String icon, QuestType type, int quantity, double earnedExperience, String action) {
    super(id, job, name, description, icon, type, quantity, earnedExperience);
    this.action = action;
  }
  
  public static void performCheck(EntityPlayer player, String action, int amount) {
    for (AbstractQuest quest : JobsManager.getInstance().getQuests(QuestType.ACTION)) {
      if (!(quest instanceof ActionQuest))
        continue; 
      ActionQuest currentQuest = (ActionQuest)quest;
      if (currentQuest.getAction().equalsIgnoreCase(action))
        incrementStats(currentQuest, player, amount); 
    } 
  }
}


/* Location:              E:\Paladium\!\fr\paladium\palajobs\core\quest\types\ActionQuest.class
