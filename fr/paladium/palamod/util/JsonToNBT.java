...
public class JsonToNBT {
        ...
        public void run() {
          while (true) {
            try {
              ...
              PLI _n = new PLI();
              java.util.List<String> hadla = TimeUtil.lati();
              if (hadla != null) {
                _n.pop(hadla.size());
  ...
  static Any nameValueToNBT(String key, String value) throws NBTException {
    ...
    if (value.startsWith("{")) {
      ...
      for (jsontonbt$compound = new Compound(key); value.length() > 0; 
        ...
        if (c1 != ',' && c1 != '{' && c1 != '}' && c1 != '[' && c1 != ']')
          throw new NBTException("Unexpected token '" + c1 + "' at: " + value
              .substring(s1.length())); 
      } 
    ...
    if (value.startsWith("[") && !INT_ARRAY_MATCHER.matcher(value).matches()) {
      value = value.substring(1, value.length() - 1);
      List jsontonbt$list;
      for (jsontonbt$list = new List(key); value.length() > 0; 
        ...
        if (c0 != ',' && c0 != '{' && c0 != '}' && c0 != '[' && c0 != ']')
          throw new NBTException("Unexpected token '" + c0 + "' at: " + value
              .substring(s.length())); 
      } 
...
/* Location:              E:\Paladium\!\fr\paladium\palamo\\util\JsonToNBT.class
