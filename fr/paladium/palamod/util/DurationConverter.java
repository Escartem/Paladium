...
public class DurationConverter {
  ...
  public static long fromStringToMillis(String duration) {
    ...
    if (duration.equalsIgnoreCase("perm") || duration.equalsIgnoreCase("perma") || duration.equalsIgnoreCase("def"))
      return 0L; 
    return -1L;
  }
  ...
  public static String fromMillisToString(long duration) {
    ...
    return (formatted.length() > 0) ? formatted.substring(0, formatted.length() - 1) : "Erreur";
  }
  
  public static String fromMillisToString(long duration, int numberOfFields) {
    if (duration == 0L)
      return "Permanant"; 
    Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    cal.setTime(new Date(duration));
    ...
    AtomicInteger currentFieldsAmount = new AtomicInteger(0);
    int years = cal.get(1) - 1970;
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (cal.get(1) > 1970) ? (years + " " + getPlurialWord("an", years) + " ") : "");
    int months = cal.get(2);
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (months > 0) ? (months + " mois ") : "");
    int days = cal.get(5) - 1;
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (days > 0) ? (days + " " + getPlurialWord("jour", days) + " ") : "");
    int hours = cal.get(11);
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (hours > 0) ? (hours + " " + getPlurialWord("heure", hours) + " ") : "");
    int minutes = cal.get(12);
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (minutes > 0) ? (minutes + " " + getPlurialWord("minute", minutes) + " ") : "");
    int seconds = cal.get(13);
    addFieldToFormattedDuration(formatted, numberOfFields, currentFieldsAmount, (seconds > 0) ? (seconds + " " + getPlurialWord("seconde", seconds) + " ") : "");
    return (formatted.length() > 0) ? formatted.substring(0, formatted.length() - 1) : "Erreur";
  }
...
/* Location:              E:\Paladium\!\fr\paladium\palamo\\util\DurationConverter.class
