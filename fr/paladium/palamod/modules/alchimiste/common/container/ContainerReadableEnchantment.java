...
public class ContainerReadableEnchantment extends Container {
  ...
  public EnchantSeed[] enchantSeeds = new EnchantSeed[3];
  
  private EntityPlayer player;
  
  public ContainerReadableEnchantment(SimpleGHandler.GuiHandlerData data) {
    this(data.getPlayer(), data.getInventory(), data.getWorld(), data.getX(), data.getY(), data.getZ());
  }
  
  public ContainerReadableEnchantment(EntityPlayer player, InventoryPlayer p_i1811_1_, World p_i1811_2_, int p_i1811_3_, int p_i1811_4_, int p_i1811_5_) {
    this.player = player;
    ...
    if (p_75130_1_ == this.tableInventory) {
      ...
      if (itemstack != null && itemstack.func_77956_u()) {
        ...
        if (!this.worldPointer.field_72995_K) {
          PalaEnchantEEP eep = PalaEnchantEEP.get((Entity)this.player);
          int i = 0;
          float power = 0.0F;
          int j;
          for (j = -1; j <= 1; j++) {
          ...
          for (j = 0; j < 3; j++) {
            ...
            enchantSeed.enchantLvl = this.enchantLevels[j];
            enchantSeed.enchantPower = (int)power;
            enchantSeed.itemStack = itemstack;
            enchantSeed.enchantDataList = new ArrayList();
            List<?> list = EnchantmentHelper.func_77513_b(this.rand, itemstack, this.enchantLevels[j]);
            boolean flag = (itemstack.func_77973_b() == Items.field_151122_aG);
            if (list != null) {
              int j1 = (flag && list.size() > 1) ? this.rand.nextInt(list.size()) : -1;
              for (int k = 0; k < list.size(); k++) {
                EnchantmentData enchantmentdata = (EnchantmentData)list.get(k);
                if (!flag || k != j1) {
                  EnchantData enchantData = new EnchantData();
                  enchantData.enchantId = enchantmentdata.field_76302_b.field_77352_x;
                  enchantData.enchantStrenght = enchantmentdata.field_76303_c;
                  enchantSeed.enchantDataList.add(enchantData);
                } 
              } 
            } 
            this.enchantSeeds[j] = eep.getOrCreateSeed(enchantSeed);
          } 
          eep.sync();
          func_75142_b();
        } 
  ...
  public boolean func_75140_a(EntityPlayer p_75140_1_, int p_75140_2_) {
    ...
    if (p_75140_1_ instanceof net.minecraft.entity.player.EntityPlayerMP && itemstack != null) {
      Palapass.processItemEnchant(p_75140_1_, itemstack);
      EnchantItemAchievement.performCheck(p_75140_1_, itemstack);
      PetBridge.callEnchantItemEvent(p_75140_1_, itemstack, this.enchantLevels[p_75140_2_]);
    } 
    if (this.enchantLevels[p_75140_2_] <= 0 || itemstack == null || (p_75140_1_.field_71068_ca < this.enchantLevels[p_75140_2_] && !p_75140_1_.field_71075_bZ.field_75098_d))
      return false; 
    if (!this.worldPointer.field_72995_K)
      if (this.enchantSeeds[p_75140_2_] != null) {
        PalaEnchantEEP eep = PalaEnchantEEP.get((Entity)this.player);
        List<EnchantmentData> list = new ArrayList<>();
        for (EnchantData eData : (this.enchantSeeds[p_75140_2_]).enchantDataList)
          list.add(new EnchantmentData(eData.enchantId, eData.enchantStrenght)); 
        boolean flag = (itemstack.func_77973_b() == Items.field_151122_aG);
        if (list != null && !list.isEmpty()) {
          p_75140_1_.func_82242_a(-this.enchantLevels[p_75140_2_]);
          if (flag)
          ...
          eep.removeSeed(this.enchantSeeds[p_75140_2_]);
  ...
  public boolean func_75145_c(EntityPlayer p_75145_1_) {
    return (this.worldPointer.func_147439_a(this.posX, this.posY, this.posZ) != Blocks.field_150381_bn) ? false : ((p_75145_1_.func_70092_e(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D) <= 64.0D));
  }
...
/* Location:              E:\Paladium\!\fr\paladium\palamod\modules\alchimiste\common\container\ContainerReadableEnchantment.class
