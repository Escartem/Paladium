...
public class ConfigManager {
  ...
  private static String file = "config/palapillage.cfg";
  ...
  public static double[] getDoubleList(String category, String key) {
    config = new Configuration(new File(file));
    try {
      config.load();
      if (config.getCategory(category).containsKey(key))
        return config.get(category, key, new double[0]).getDoubleList(); 
    } catch (Exception e) {
      System.out.println("Cannot load configuration file!");
    } finally {
      config.save();
  ...
  public static void writeConfig(String category, String key, double[] value) {
    ...
    try {
      config.load();
      double[] set = config.get(category, key, value).getDoubleList();
      config.getCategory(category).get(key).set(value);
    } catch (Exception e) {
      System.out.println("Cannot load configuration file!");
    } finally {
...
/* Location:              E:\Paladium\!\fr\paladium\palamod\modules\pillage\ConfigManager.class
