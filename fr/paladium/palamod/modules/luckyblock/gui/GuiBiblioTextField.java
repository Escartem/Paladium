...
public class GuiBiblioTextField extends Gui {
  ...
  public void writeText(String par1Str) {
    ...
    int i = (this.cursorPosition < this.selectionEnd) ? this.cursorPosition : this.selectionEnd;
    int j = (this.cursorPosition < this.selectionEnd) ? this.selectionEnd : this.cursorPosition;
    int k = this.maxStringLength * 4 - this.text.length() - i - this.selectionEnd * 2;
    boolean flag = false;
    if (this.text.length() > 0)
      s1 = s1 + this.text.substring(0, i); 
    if (k < s2.length()) {
      try {
        s1 = s1 + s2.substring(0, k + 2);
        l = k;
      } catch (Exception e) {
        l = k;
      } 
  ...
  public boolean mouseClicked(int par1, int par2, int par3) {
    ...
    if (this.isFocused && par3 == 0) {
      ...
      if (this.enableBackgroundDrawing)
        l -= 4; 
      String s = this.fontRenderer.func_78269_a(this.text.substring(this.lineScrollOffset), 
          getWidth());
      setCursorPosition(this.fontRenderer.func_78269_a(s, l).length() + this.lineScrollOffset);
      return true;
    } 
    return false;
  }
  ...
  public void drawTextBox() {
    if (getVisible()) {
      ...
      if (s.length() > 0) {
        ...
        j1 = this.fontRenderer.func_78276_b(s1, l, i1, i);
      } 
      ...
      if (s.length() > 0 && flag && j < s.length())
        this.fontRenderer.func_78276_b(s.substring(j), j1, i1, i); 
      if (flag1)
        if (flag2) {
          Gui.func_73734_a(k1, i1 - 1, k1 + 1, i1 + 1 + this.fontRenderer.field_78288_b, -3092272);
        } else {
          this.fontRenderer.func_78276_b("_", k1, i1, i);
        }  
      if (k != j) {
        int l1 = l + this.fontRenderer.func_78256_a(s.substring(0, k));
        drawCursorVertical(k1, i1 - 1, l1 - 1, i1 + 1 + this.fontRenderer.field_78288_b);
      } 
    } 
  }
  ...
  private void drawCursorVertical(int par1, int par2, int par3, int par4) {
    ...
    tessellator.func_78377_a(par1, par4, 0.0D);
    tessellator.func_78377_a(par3, par4, 0.0D);
    tessellator.func_78377_a(par3, par2, 0.0D);
    tessellator.func_78377_a(par1, par2, 0.0D);
...
/* Location:              E:\Paladium\!\fr\paladium\palamod\modules\luckyblock\gui\GuiBiblioTextField.class
