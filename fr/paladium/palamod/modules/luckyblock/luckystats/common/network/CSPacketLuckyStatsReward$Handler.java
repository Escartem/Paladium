...
public class Handler implements IMessageHandler<CSPacketLuckyStatsReward, IMessage> {
  public IMessage onMessage(CSPacketLuckyStatsReward message, MessageContext ctx) {
    EntityPlayerMP player = (ctx.getServerHandler()).field_147369_b;
    if ((LuckyType.values()).length > CSPacketLuckyStatsReward.access$000(message)) {
      LuckyType luckyType = LuckyType.values()[CSPacketLuckyStatsReward.access$000(message)];
      (new PlayerLuckyStats((EntityPlayer)player)).load(stats -> {
            if (((Integer)((List<Integer>)stats.getRewards().get(luckyType)).get(CSPacketLuckyStatsReward.access$100(message))).intValue() == 0) {
              if ((stats.getStats().keySet().stream().filter(()).toArray()).length >= Math.floor((LuckyEvents.valuesEvents(luckyType).size() / 100.0F * ((CSPacketLuckyStatsReward.access$100(message) + 1) * 25)))) {
                AtomicReference<LuckyStatsRewardLevel> reward = new AtomicReference<>(LuckyStatsRewardManager.getRewards(luckyType, CSPacketLuckyStatsReward.access$100(message)));
                for (LuckyStatsRewardLevel r : LuckyStatsRewardManager.getRewards().get(luckyType)) {
                  if (r.getLevel() != CSPacketLuckyStatsReward.access$100(message))
                    continue; 
                  reward.set(r);
                } 
                if (reward.get() == null) {
                ...
                stats.setReward(luckyType, CSPacketLuckyStatsReward.access$100(message), 1);
                stats.save((), ());
              } 
            } else {
