  ...
  static Any nameValueToNBT(String key, String value) throws NBTException {
    ...
    if (value.startsWith("{")) {
      ...
      for (jsontonbt$compound = new Compound(key); value.length() > 0; value = value.substring(s1.length() + 1)) {
        String s1 = nextNameValuePair(value, true);
        if (s1.length() > 0)
    ...
    if (value.startsWith("[") && !INT_ARRAY_MATCHER.matcher(value).matches()) {
      ...
      for (jsontonbt$list = new List(key); value.length() > 0; value = value.substring(s.length() + 1)) {
        String s = nextNameValuePair(value, false);
        if (s.length() > 0)
    ...
    public NBTBase parse() throws NBTException {
      try {
        if (DOUBLE.matcher(this.jsonValue).matches())
          return (NBTBase)new NBTTagDouble(Double.parseDouble(this.jsonValue.substring(0, this.jsonValue.length() - 1))); 
        if (FLOAT.matcher(this.jsonValue).matches())
          return (NBTBase)new NBTTagFloat(Float.parseFloat(this.jsonValue.substring(0, this.jsonValue.length() - 1))); 
        if (BYTE.matcher(this.jsonValue).matches())
          return (NBTBase)new NBTTagByte(Byte.parseByte(this.jsonValue.substring(0, this.jsonValue.length() - 1))); 
        if (LONG.matcher(this.jsonValue).matches())
          return (NBTBase)new NBTTagLong(Long.parseLong(this.jsonValue.substring(0, this.jsonValue.length() - 1))); 
        if (SHORT.matcher(this.jsonValue).matches())
          return (NBTBase)new NBTTagShort(Short.parseShort(this.jsonValue.substring(0, this.jsonValue.length() - 1))); 
...
/* Location:              E:\Paladium\!\fr\paladium\palaforgeutils\lib\itemstack\JsonToNBT.class
