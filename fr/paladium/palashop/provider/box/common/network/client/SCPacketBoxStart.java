...
public class SCPacketBoxStart extends ForgePacket {
  @PacketData
  private int entityId;
  
  @PacketData
  private SCPacketBoxOpenData data;
  
  public SCPacketBoxStart() {}
  
  public SCPacketBoxStart(int entityId, SCPacketBoxOpenData data) {
  ...
  public void processClient() {
    ...
    (new Thread(() -> {
          while (System.currentTimeMillis() - start < 30000L) {
            ...
            if (box.getClientState().isActive() && !box.getClientState().isWaiting())
              return; 
            box.getClientState().start(this.data.getPlayerName(), this.data.isFast(), this.data.getAvailableRewards(), this.data.getRewards(), this.data.getCurrentSpin());
            return;
          } 
        }"PalaShop/SCPacketBoxStart"))
      
      .start();
  }
  
  public static class SCPacketBoxOpenData {
