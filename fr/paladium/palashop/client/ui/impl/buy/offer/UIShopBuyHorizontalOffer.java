...
public class UIShopBuyHorizontalOffer extends UIShopPage implements UIShopPage.IBackableUI {
  ...
  public UIShopBuyHorizontalOffer(@NonNull UIShopPage back, @NonNull ConditionalBuyableObject<ShopOffer> buyable) {
  ...
  public void init() {
    ...
    RectNode.create(42.0D, 252.0D, 1836.0D, 2.0D)
      ...
      .attach((UI)this);
    ((RectNode)RectNode.create(42.0D, 306.0D, 1062.0D, 438.0D)
      .color(ColorConstant.GRAY)
      .effect((NodeEffect)RoundedNodeEffect.create(20.0F))
      .body(rect -> {
          if (((ShopOffer)((ConditionalBuyableObject)this.buyableSignal.getOrDefault()).getObject().getItem()).getThumbnail() != null)
            ImageNode.create(0.0D, 0.0D, rect.getWidth(), rect.getHeight()).resource(Resource.of(((ShopOffer)((ConditionalBuyableObject)this.buyableSignal.getOrDefault()).getObject().getItem()).getThumbnail())).effect(rect.getEffect(RoundedNodeEffect.class)).attach((Node)rect); 
          ImageNode.create(rect.aw(-22.0D), 22.0D).resource(ShopRarityConstant.getResource(((ShopOffer)((ConditionalBuyableObject)this.buyableSignal.getOrDefault()).getObject().getItem()).getRarity())).width(107.0D).anchorX(Align.END).attach((Node)rect);
        })).attach((UI)this);
    FlexNode.horizontal(1171.0D, 306.0D, 56.0D)
      .margin(20.0D)
      .body(flex -> {
    ...
    TextNode.create(1171.0D, 389.0D)
      ...
      .attach((UI)this);
    TextNode.create(1171.0D, 479.0D)
    ...
    if (((ShopOffer)((ConditionalBuyableObject)this.buyableSignal.getOrDefault()).getObject().getItem()).getConditionalShopItems() != null && !((ShopOffer)((ConditionalBuyableObject)this.buyableSignal.getOrDefault()).getObject().getItem()).getConditionalShopItems().isEmpty()) {
      TextNode.create(42.0D, 762.0D)
        .text(Text.create("cette offre contient :", TextInfo.create((IFont)PaladiumFont.MONTSERRAT_SEMI_BOLD, 20.0F, Color.WHITE)))
        .attach((UI)this);
      ContainerNode.create(42.0D, 804.0D, 1062.0D, 244.0D)
        .overflow(OverflowProperty.SCROLL)
        .scrollbar((ScrollbarNode)ScrollbarNode.horizontal(0.0D, 255.0D, 1062.0D))
        .body(container -> FlexNode.horizontal(0.0D, 0.0D, container.getHeight()).margin(21.0D).body(()).watch(this.buyableSignal, new WatchProperty[] { WatchProperty.CLEAR_CHILDREN, WatchProperty.BODY }).attach(container)).attach((UI)this);
    } 
...
/* Location:              E:\Paladium\!\fr\paladium\palashop\clien\\ui\impl\buy\offer\UIShopBuyHorizontalOffer.class
