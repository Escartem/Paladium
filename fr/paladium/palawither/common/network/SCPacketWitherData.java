...
import fr.paladium.palawither.client.overlay.UIWitherOverlay;
...
public class SCPacketWitherData extends ForgePacket {
  @PacketData
  private WitherData witherData;
  
  public SCPacketWitherData() {}
  
  private SCPacketWitherData(@NonNull TileEntityWitherReceptacle receptacle) {
    if (receptacle == null)
      throw new NullPointerException("receptacle is marked non-null but is null"); 
    this.witherData = new WitherData(receptacle);
  }
  
  private SCPacketWitherData(@NonNull IWither wither) {
    this(wither, wither.getEntity());
    if (wither == null)
      throw new NullPointerException("wither is marked non-null but is null"); 
  }
  
  private SCPacketWitherData(@NonNull IWither wither, @NonNull EntityLivingBase entity) {
    if (wither == null)
      throw new NullPointerException("wither is marked non-null but is null"); 
    if (entity == null)
      throw new NullPointerException("entity is marked non-null but is null"); 
    this.witherData = new WitherData(wither, entity);
  }
  
  public static void update(@NonNull TileEntityWitherReceptacle receptacle) {
    if (receptacle == null)
      throw new NullPointerException("receptacle is marked non-null but is null"); 
    if (receptacle.getWither() == null)
      return; 
    SCPacketWitherData packet = new SCPacketWitherData(receptacle);
    PlayerSelector.ALL().apply(player -> {
          if (player.func_70011_f(receptacle.field_145851_c, receptacle.field_145848_d, receptacle.field_145849_e) > receptacle.getWither().getViewDistance() * 2.0D)
            return; 
          packet.send(player);
        });
  }
  
  public static void update(@NonNull IWither wither) {
    if (wither == null)
      throw new NullPointerException("wither is marked non-null but is null"); 
    update(wither, wither.getEntity());
  }
  
  public static void update(@NonNull IWither wither, @NonNull EntityLivingBase entity) {
    if (wither == null)
      throw new NullPointerException("wither is marked non-null but is null"); 
    if (entity == null)
      throw new NullPointerException("entity is marked non-null but is null"); 
    if (wither.hasUpgrade(ItemsRegister.FURTIVE_WITHER_UPGRADE))
      return; 
    SCPacketWitherData packet = new SCPacketWitherData(wither);
    PlayerSelector.ALL().apply(player -> {
          if (player.func_70011_f(entity.field_70165_t, entity.field_70163_u, entity.field_70161_v) > wither.getViewDistance() * 2.0D)
            return; 
          packet.send(player);
        });
  }
  ...
  public void processClient() {
    UIWitherOverlay ui = (UIWitherOverlay)ZUI.getUI(UIWitherOverlay.class);
    if (ui == null) {
      ui = new UIWitherOverlay();
      ui.add(this.witherData);
      ZUI.open((UI)ui);
      return;
    } 
    ui.add(this.witherData);
