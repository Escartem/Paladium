...
import fr.paladium.pet.common.capture.CaptureCategory;
import fr.paladium.pet.common.capture.CaptureSection;
...
public class UICapture extends UI {
  ...
  private static final Color BAR_COLOR = Color.decode("#323232");
  ...
  public UICapture(CaptureCategory category, CageStatus status, int x, int y, int z) {
  ...
  public void func_73866_w_() {
    ...
    addNode((ANode)new MinecraftTitleNodeLabel(width(23.0F), height(27.59F), this.title));
  ...
  public void postDraw(int mouseX, int mouseY, float ticks) {
    ...
    double barY = height(53.33F);
    double barWidth = width(53.0F);
    GuiUtils.drawCenteredStringWithCustomFont(this.field_146297_k, this.subtitle, width(50.0F), height(42.0F), Color.WHITE, Fonts.PIXEL_NES.getFont(), 70);
    GuiUtils.drawCenteredStringWithCustomFont(this.field_146297_k, this.clickSpaceText, width(50.0F), height(46.0F), Color.decode("#41414E"), Fonts.PIXEL_NES.getFont(), 20);
    ...
    List<CaptureSection> showedSections = new ArrayList<>();
    double darkY = height(65.0F);
    double darkX = barX;
    GuiUtils.drawRect(darkX, darkY, darkX + width(1.0F), darkY + width(1.0F), BAR_COLOR);
    darkX += width(1.5F);
    GuiUtils.drawStringWithCustomFont(this.field_146297_k, "-150", darkX, darkY + height(0.2F), Color.WHITE, Fonts.PIXEL_NES.getFont(), 40);
    for (int i = 1; i < this.category.getSections().size() + 1; i++) {
      CaptureSection section = this.category.getSections().get(i - 1);
      if (!showedSections.contains(section)) {
        int value = section.getValue();
        String sign = (section.getValue() > 0) ? "+" : "";
        double ox = barX + (width(8.0F) * (i / 2));
        double oy = (height(65.0F) + height(3.5F) * (i % 2));
        GuiUtils.drawRect(ox, oy, ox + width(1.0F), oy + width(1.0F), section.getColor());
        ox += width(1.5F);
        ox += GuiUtils.drawStringWithCustomFont(this.field_146297_k, sign + String.valueOf(value), ox, oy + height(0.2F), Color.WHITE, Fonts.PIXEL_NES.getFont(), 40);
  ...
  protected void func_73869_a(char c, int keyCode) {
    if (keyCode == 57) {
      ...
      if (focusedSection != null) {
        String soundName = (focusedSection.getValue() <= 0) ? SoundUtils.ITEM_BREAK.getSoundName() : "random.orb";
        ...
        sendPacket(focusedSection.getValue());
        this.field_146297_k.func_147108_a(null);
      } else {
        this.field_146297_k.func_147118_V().func_147682_a((ISound)PositionedSoundRecord.func_147674_a(new ResourceLocation(SoundUtils.ITEM_BREAK.getSoundName()), 5.0F));
        sendPacket(-150);
